...
function App() {
  ...
  const [reportsAnimationKey, setReportsAnimationKey] = useState(0)
...
function HomePage({
  ...
  const pagedActivity = recentActivity.slice(currentActivityPage * activityPageSize, currentActivityPage * activityPageSize + activityPageSize)
  ...
  useEffect(() => {
    setActivityPage((current) => Math.min(current, activityPageCount - 1))
  }, [activityPageCount])
    ...
    <section className="home-page">
      ...
      >
        <div className="home-hero-copy">
          <img alt="LobiLend" className="home-hero-wordmark" src={brandLogoText} />
          <div>
      ...
      <section className={`panel home-panel report-accordion-panel ${isActivityOpen ? 'open' : ''}`}>
        <button
        ...
        >
          <span>
            ...
            <small>{recentActivity.length} audit events</small>
          </span>
          <em>{isActivityOpen ? '-' : '+'}</em>
        </button>
        {isActivityOpen && (
          <div className="report-accordion-body">
            <div className="activity-list">
              ...
              {pagedActivity.length === 0 && <p className="state-message">No recent activity yet.</p>}
            </div>
            <ListPagination
              currentPage={currentActivityPage}
              itemLabel="Recent activity"
...
function AccordionPanel({
  ...
  return (
    <section className={`panel accordion-panel ${className} ${isOpen ? 'open' : 'collapsed'}`}>
  ...
  return (
    <aside className="panel detail-panel loan-pipeline-detail">
      <div className="detail-header">
        ...
        <button className="detail-panel-close" type="button" aria-label="Close loan details" onClick={onClose}>
          x
        </button>
      </div>

      <div className="detail-primary-actions">
        <button className="detail-action" type="button" onClick={() => onOpenDetails(selected.loanNumber)}>More Details</button>
  ...
  return (
    <section className="loan-detail-page">
      ...
      <section className="loan-detail-grid wide">
        <AccordionPanel title="Needs" subtitle={`${actions.length} total action records`}>
          <LoanNeedsGroups actions={actions} />
        </AccordionPanel>

        <AccordionPanel title="Create action" subtitle="Manual or template-based work.">
          ...
          <div className="follow-up-box template-generate-box">
            ...
            <label>
              Action template
              <select disabled={disabled || templates.length === 0} defaultValue={selectedGenerationTemplateId}>
      ...
      <AccordionPanel title="Edit loan" subtitle="Update tracker fields, contacts, stage, and ICD state.">
      ...
      <section className="loan-detail-grid">
        <AccordionPanel className="activity-feed" title="Actions" subtitle={`${actions.length} action records`}>
          {actions.map((action) => (
            <button className="context-row" key={action.id} type="button" onClick={() => onOpenAction(action.id)}>
              <span>
                <strong>{action.title}</strong>
        ...
        <AccordionPanel className="activity-feed" title="Recent notes">
      ...
      <AccordionPanel className="activity-feed" title="History">
...
function AdminTemplatesPage({
  ...
  function toggleAdminSection(section: AdminAccordionSection) {
  ...
  return (
    <section className="content-grid admin-grid admin-accordion-grid">
      <section className={`panel admin-accordion-panel ${isTemplatesOpen ? 'open' : ''}`}>
        <button
          aria-expanded={isTemplatesOpen}
          className="admin-accordion-trigger"
          type="button"
          onClick={() => toggleAdminSection('templates')}
        >
          <span>
            <strong>Action templates</strong>
            <small>{filteredTemplates.length} visible, {templateDetail ? `${templateDetail.items.length} saved items` : 'draft ready'}</small>
          </span>
          <em>{isTemplatesOpen ? '-' : '+'}</em>
        </button>

        {isTemplatesOpen && (
          <div className="admin-accordion-body admin-template-workbench">
            ...
            <form className="template-editor-panel admin-section-block" onSubmit={onSubmit}>
              <div className="panel-header">
                <div>
                  <h2>{form.id ? 'Edit template' : 'New template'}</h2>
                  <p>{templateDetail ? `${templateDetail.items.length} saved items` : 'Draft'}</p>
                </div>
                <button disabled={isTemplateSaveDisabled} type="submit">{disabled ? 'Saving...' : 'Save template'}</button>
              </div>
              ...
              {form.items.length > 0 ? (
                ...
                <p className="state-message">Click Add item to start this template.</p>
              )}
      ...
      <section className={`panel admin-accordion-panel ${isUsersOpen ? 'open' : ''}`}>
        <button
        ...
        >
          <span>
            ...
            <small>{activeUsers.length} active, {inactiveUsers.length} inactive</small>
          </span>
          <em>{isUsersOpen ? '-' : '+'}</em>
        </button>

        {isUsersOpen && (
          <div className="admin-accordion-body admin-user-list">
      ...
      <section className={`panel admin-accordion-panel ${isInviteOpen ? 'open' : ''}`}>
        <button
          aria-expanded={isInviteOpen}
          className="admin-accordion-trigger"
          type="button"
          onClick={() => toggleAdminSection('invite')}
        >
          <span>
            <strong>Invite user</strong>
            <small>Send a workspace setup email</small>
          </span>
          <em>{isInviteOpen ? '-' : '+'}</em>
        </button>

        {isInviteOpen && (
          <form className="admin-accordion-body admin-user-form" onSubmit={onSubmitUser}>
            <div>
              <h3>Invite user</h3>
...
function ReportsPage({
  ...
  return (
    <section className="reports-page">
      ...
      <section className={`panel report-panel report-accordion-panel ${isActivityOpen ? 'open' : ''}`}>
        <button
          aria-expanded={isActivityOpen}
          className="report-accordion-trigger"
          type="button"
        ...
        >
          <span>
            <strong>Recent activity</strong>
            <small>{summary.recentActivity.length} audit events</small>
          </span>
          <em>{isActivityOpen ? '-' : '+'}</em>
        </button>
        {isActivityOpen && (
          <div className="report-accordion-body activity-list">
        ...
        {items.map((item) => (
          <div className="breakdown-row" key={item.label}>
            ...
            <div aria-hidden="true">
              <i style={{ '--bar-width': `${Math.max(8, (item.value / maxValue) * 100)}%` } as CSSProperties} />
            </div>
          </div>
        ))}
  ...
  return (
    <aside className="panel detail-panel customer-detail-panel">
      <div className="detail-header">
        ...
        <button className="detail-panel-close" type="button" aria-label="Close customer details" onClick={onClose}>
          x
        </button>
      </div>

      <div className="detail-primary-actions">
        <button className="detail-action" type="button" onClick={() => onOpenDetails(selected.id)}>More Details</button>
      </div>
...
function CustomerDetailPage({
  ...
  return (
    <section className="customer-detail-page">
      ...
      <AccordionPanel className="edit-box" title="Profile" subtitle="Borrower contact and record status.">
      ...
      <AccordionPanel className="add-loan-box" title="Add loan" subtitle="Create another file for this customer.">
      ...
      <AccordionPanel className="activity-feed" title="Loans" subtitle={`${loanCount} customer files`}>
      ...
      <AccordionPanel className="activity-feed" title="Open actions" subtitle={`${openActionCount} active items`}>
...
function LoanContextPanel({
  ...
  onClose: () => void
  onOpenDetail: () => void
  onViewLoan: (loanNumber: string) => void
}) {
  ...
  return (
    <aside className="panel detail-panel">
      <div className="detail-header">
        ...
        <button className="detail-panel-close" type="button" aria-label="Close action details" onClick={onClose}>
          x
        </button>
      </div>

      <div className="detail-primary-actions">
        <button className="detail-action" type="button" onClick={onOpenDetail}>More Details</button>
      ...
      <dl>
        ...
        <div>
          ...
          <dd className="detail-field-action">
            <span>{action.loanNumber}</span>
            <button className="secondary" type="button" onClick={() => onViewLoan(action.loanNumber)}>
              View Loan
            </button>
          </dd>
        </div>
...
function ActionDetailPage({
  ...
  return (
    <section className="action-detail-page">
      ...
      <AccordionPanel className="email-draft-box" title="Borrower email" subtitle="Generate a draft for the selected condition.">
        <button
          className="secondary"
      ...
      <AccordionPanel className="follow-up-box" title="New follow-up" subtitle="Add another condition or partner touch to this loan.">
        <label>
          Follow-up title
          <input
            disabled={disabled}
      ...
      <AccordionPanel className="follow-up-box template-generate-box" title="Generate from template" subtitle="Create missing standardized actions for this loan.">
        <label>
          Action template
          <select
            disabled={disabled || templates.length === 0}
      ...
      <AccordionPanel className="reassign-box" title="Reassign" subtitle="Move ownership to another active user.">
        ...
        <label htmlFor="reassignReason">Reassignment reason</label>
        <textarea
          id="reassignReason"
          disabled={disabled}
      ...
      <AccordionPanel className="note-box" title="File note" subtitle="Add borrower, title, or realtor context.">
      ...
      <AccordionPanel className="reschedule-box" title="Reschedule" subtitle="Move the due date and capture the reason.">
      ...
      <AccordionPanel className="cancel-box" title="Cancel action" subtitle="Close work that no longer applies.">
